...
def main():
    ...
    """
    # Hardcoded the XML file name and tag here
    xml_file_path = "p00734.xml"
    tag = "entry"  # Change this to your interested tag

    # Check if the file exists
    if not os.path.isfile(xml_file_path) or not xml_file_path.endswith("xml"):
        raise Exception("The input file is not exist or not a valid XML file.")
    
    # Initialize the parser and parse the XML file
    xml_parser = largeXMLDealer()
    count = xml_parser.parse(xml_file_path, tag)
    print("Parsed %10d XML elements." % count)
'''
