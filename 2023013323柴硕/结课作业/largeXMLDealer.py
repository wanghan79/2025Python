...
class largeXMLDealer:
    ...
    def __init__(self, fileName, elemTag):
        ...
        self.func4Element = None
        self.namespace = self._getNamespace(fileName)

    def __call__(self, func):
        self.func4Element = func
        @functools.wraps(func)
        def wrapper(*args, **kwargs):
            count = 0
            for elem in args[0]:
                print(f'[*]Parsing "{elem}" now...')
                count += self.parse(self.fileName,elem,func)
            return count
        return wrapper
...
def main():
    ...
    # Call XML parser
    largXML = largeXMLDealer(filePath, options.tag)
    count = largXML.parse(filePath, options.tag,func4Element=dealwithElement)
